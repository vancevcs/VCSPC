...
void VCSMenuScreen::AddSaveRows(UI::ViewGroup *parent, bool deleting) {
	...
	for (const VCS::SaveSlot &slot : slots) {
		...
		if (deleting) {
			...
			row->OnClick.Add([this, index, name](UI::EventParams &e) {
				auto di = GetI18NCategory(I18NCat::DIALOG);
				screenManager()->push(new UI::MessagePopupScreen(
					"Delete " + name + "?", "This cannot be undone.",
					di->T("Delete"), di->T("Cancel"), [this, index](bool result) {
						if (!result) {
							return;
						}
						// Recorded, not done: see pendingDelete_. The save is deleted and the
						// page rebuilt on the next update, once this popup has finished.
...
void VCSMenuScreen::OnNewGame(UI::EventParams &e) {
	auto di = GetI18NCategory(I18NCat::DIALOG);
	screenManager()->push(new UI::MessagePopupScreen("Start a new game?",
		"Anything you have not saved will be lost.", di->T("Yes"), di->T("Cancel"),
		[this](bool result) {
			if (!result) {
				return;
			}
