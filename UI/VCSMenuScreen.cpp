...
// How far left of centre the save list starts. Wide enough for the longest mission name the game
// has to offer plus its slot number, so the column reads as centred rather than as pushed left.
static constexpr float kSaveListInset = 240.0f;
...
// The question on a confirmation page, and the line under it. Set in the UI sans rather than in
// the display face for the reason the controls listing is: Pricedown sets four words across a
// screen beautifully and a sentence badly, and a question with a save's name in it is a sentence.
static const FontStyle kConfirmFont(FontFamily::SansSerif, 28, FontStyleFlags::Default);
static const FontStyle kConfirmDetailFont(FontFamily::SansSerif, 20, FontStyleFlags::Default);
...
void VCSMenuItem::Draw(UIContext &dc) {
	...
	if (!option_) {
		if (leftAlignX_ >= 0.0f) {
			// A list row: the label starts where every other label on the page starts. See
			// SetLeftAligned.
			dc.DrawTextShadow(label_, leftAlignX_, bounds_.centerY(), color,
				ALIGN_VCENTER | ALIGN_LEFT);
			hitBounds_ = Bounds(leftAlignX_ - slack, bounds_.y, labelW + slack * 2.0f, bounds_.h);
...
void VCSMenuScreen::GoToConfirm(const char *titleKey, std::string question, std::string detail,
	...
	confirmTitle_ = titleKey;
	confirmQuestion_ = std::move(question);
	confirmDetail_ = std::move(detail);
	confirmAction_ = std::move(action);
	confirmParent_ = page_;
	GoToPage(VCSMenuPage::Confirm);
}
...
// The save list, ours rather than the firmware's.
//
// Every row is a slot the GAME has, not a directory that happens to exist: eight of them, empty
// ones included, because a save always overwrites one of eight and a list that hid the empties
// would renumber itself as the player filled them.
//
// The row's label is the mission the save is named after and its help line is everything else the
// game wrote about it - district, safe house, day, money, percentage, and when it was written.
// That is the whole reason this list can be one line per save: the explanation already has a home
// at the bottom of this screen, and the game wrote the words.
void VCSMenuScreen::AddSaveRows(UI::ViewGroup *parent, bool deleting) {
	using namespace UI;
	...
	for (const VCS::SaveSlot &slot : slots) {
		...
		VCSMenuItem *row = parent->Add(new VCSMenuItem(label,
			new LinearLayoutParams(FILL_PARENT, kRowHeight)));
		row->SetHelp(slot.present ? slot.summary : "Nothing saved in this slot.");
		// One x for all eight, a little left of centre so the block still sits in the middle of
		// the screen. BACK, added by the page below, stays centred like every other action row.
		row->SetLeftAligned(ScreenCenterX() - kSaveListInset);
		// An empty slot is shown and not offered. Greyed out rather than left off the list, so
		// the eight slots keep their numbers.
		row->SetEnabled(slot.present);
		...
		if (deleting) {
			...
			row->OnClick.Add([this, index, name, when](UI::EventParams &e) {
				GoToConfirm(PageTitle(page_),
					"Are you sure you wish to delete this save file?", name + " - " + when,
					[this, index]() {
						// Recorded, not done: see pendingDelete_. The save goes and the list is
						// rebuilt on the next update, off this row's own click.
						pendingDelete_ = index;
					});
			});
		} else {
			row->OnClick.Add([this, index](UI::EventParams &e) {
				// Queue and get out of the way, the same shape as every other row that reaches
				// the running game: this screen pauses the emulator, so nothing can be pressed
				// at the game until it closes. The loading screen goes up as the request is
				// made, so what the player sees next is a loading screen rather than the game's
				// own menu being walked.
				VCS::RequestLoadSlot(index);
				TriggerFinish(DR_CANCEL);
			});
		}
		rows_.push_back(row);
...
void VCSMenuScreen::CreateViews() {
	...
	if (page_ == VCSMenuPage::Root) {
		...
		// the two that show a list show OURS - see AddSaveRows. New game still goes through the
		// game's own entry, because only the game can start one.
		{
			VCSMenuItem *row = list->Add(new VCSMenuItem("NEW GAME",
				new LinearLayoutParams(FILL_PARENT, kRowHeight)));
			row->SetHelp("Start the story again from the beginning.");
			row->OnClick.Handle(this, &VCSMenuScreen::OnNewGame);
			rows_.push_back(row);
		}
		AddPageRow(list, "LOAD GAME", VCSMenuPage::LoadGame);
		AddPageRow(list, "DELETE GAME", VCSMenuPage::DeleteGame);
		AddBackRow(list);
	} else if (page_ == VCSMenuPage::Confirm) {
		// Cancel first, and focused, which is where this menu's focus lands. The game's own
		// confirm page puts No above Yes for the same reason: the default answer to a question
		// nobody has read yet should be the one that changes nothing.
		VCSMenuItem *cancel = list->Add(new VCSMenuItem("NO",
			new LinearLayoutParams(FILL_PARENT, kRowHeight)));
		cancel->SetHelp("Go back without changing anything.");
		cancel->OnClick.Add([this](UI::EventParams &e) {
			GoToPage(confirmParent_);
		});
		rows_.push_back(cancel);

		VCSMenuItem *yes = list->Add(new VCSMenuItem("YES",
			new LinearLayoutParams(FILL_PARENT, kRowHeight)));
		yes->SetHelp(confirmDetail_);
		yes->OnClick.Add([this](UI::EventParams &e) {
			// The action only ever RECORDS what to do - see the pending flags in update(). A row
			// that rebuilt this screen or finished it from inside its own click handler is the
			// bug that made the whole menu stop taking input.
			if (confirmAction_) {
				confirmAction_();
			}
		});
		rows_.push_back(yes);
	} else if (page_ == VCSMenuPage::LoadGame) {
		AddSaveRows(list, false);
		AddBackRow(list);
...
void VCSMenuScreen::update() {
	...
	// A popup's answer, acted on here rather than in the callback that recorded it - the popup is
	// gone by now, so the screen stack is ours to change.
	if (pendingNewGame_) {
		pendingNewGame_ = false;
		// Queue and close, like every other row that reaches the running game. The loading
		// screen goes up with the request, so the walk that follows is not on screen.
		VCS::RequestNewGame();
		TriggerFinish(DR_CANCEL);
		return;
	}
	...
	if (pendingDelete_ >= 0) {
		...
		pendingDelete_ = -1;
		VCS::DeleteSave(index);
		// The list is built from the memory stick, so rebuilding it is what makes the row go
		// away with the save.
		RecreateViews();
	}
...
void VCSMenuScreen::DrawBackground(UIContext &dc) {
	...
	// The question, under the heading and hard against the same left margin - which is where
	// this series has always put it, and it is why it is not centred: a centred paragraph of two
	// unequal lines reads as a poster, and this is a sentence being asked of somebody. It wraps
	// inside a box rather than being written as two strings, so a save's name can be as long as
	// it likes.
	if (page_ == VCSMenuPage::Confirm) {
		const Bounds box(kTitleLeft, kTitleTop + kTitleHeight + 8.0f,
			bounds.w - kTitleLeft * 2.0f, 120.0f);
		dc.SetFontStyle(kConfirmFont);
		dc.DrawTextShadowRect(confirmQuestion_, box, kHintColor,
			ALIGN_TOP | ALIGN_LEFT | FLAG_WRAP_TEXT);
		if (!confirmDetail_.empty()) {
			const Bounds under(box.x, box.y + 84.0f, box.w, 60.0f);
			dc.SetFontStyle(kConfirmDetailFont);
			dc.DrawTextShadowRect(confirmDetail_, under, colorAlpha(kHintColor, 0.7f),
				ALIGN_TOP | ALIGN_LEFT | FLAG_WRAP_TEXT);
		}
	}
...
void VCSMenuScreen::OnExitGame(UI::EventParams &e) {
	GoToConfirm("quit",
		"Are you sure you want to quit? All progress since the last save game will be lost. "
		"Proceed?", "", [this]() {
			pendingExit_ = true;
		});
}

void VCSMenuScreen::OnNewGame(UI::EventParams &e) {
	GoToConfirm(PageTitle(page_),
		"Are you sure you want to start a new game? All current game progress will be lost. "
		"Proceed?", "", [this]() {
			// Recorded, not done: see pendingNewGame_.
			pendingNewGame_ = true;
		});
}
...
void VCSMenuScreen::OnQuitApp(UI::EventParams &e) {
	GoToConfirm("quit", "Are you sure you want to quit to the desktop? Proceed?", "", [this]() {
		pendingQuitApp_ = true;
	});
}
